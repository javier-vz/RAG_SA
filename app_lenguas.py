...
def retrieve(query: str, motor: dict, top_k: int = 10, alpha: float = 0.6):
    ...
    lex_raw: dict = defaultdict(float)
    for tok in q_tokens:
    ...
    # Exact label bonus
    q_lower = query.lower()
    entities = cargar_grafo._instance_cache if hasattr(cargar_grafo, "_instance_cache") else {}
    for eid in id_map:
        ent = st.session_state.get("_entities", {}).get(eid)
...
def build_context(entity_ids: list, entities: dict, max_chars: int = 2500) -> str:
    ...
    for eid in entity_ids[:5]:
        ...
        name = ent["label"]
        lines = [f"[{ent['type']}] {name}"]

        if ent["comment"]:
            lines.append(f"  {ent['comment'][:300]}")
        elif ent["wiki"]:
            lines.append(f"  {ent['wiki'][:200]}")

        if ent["iso"]:       lines.append(f"  ISO 639-3: {ent['iso']}")
        if ent["countries"]: lines.append(f"  Countries: {ent['countries']}")
        if ent["speakers"]:
            spk = ent["speakers"]
            lines.append(f"  Speakers: {'extinct' if spk == '0' else spk}")

        # Family names
        fam_names = [entities[f]["label"] for f in ent["families"][:3] if f in entities]
        if fam_names:
            lines.append(f"  belongs to family: {', '.join(fam_names)}")

        # Members (for Family nodes)
        if ent["type"] == "Family" and ent["members"]:
            mnames = [entities[m]["label"] for m in ent["members"][:12] if m in entities]
            if mnames:
                lines.append(f"  member languages: {', '.join(mnames)}")
...
Be precise and factual. If the context contains family, geographic, or vitality
information, use it directly. If truly no relevant information exists, say so briefly.

Format: 2–5 sentences, direct and informative. Respond in the same language as the question."""
